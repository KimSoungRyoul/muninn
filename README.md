...
| [`huginnAgentRuntime/`](./huginnAgentRuntime/) | 에이전트 실행 1회마다 도는 컨테이너 이미지 (`claude-code` 런타임) | Dockerfile + `claude_skill.sh` + Python (`claude-agent-sdk`) |
| [`huginnSelfRuntime/`](./huginnSelfRuntime/) | 옵션 런타임 (`huginn-self`) — 비-Claude/오픈 모델을 OpenAI 호환 API 로 직접 구동 | Go (`main.go` + `agent`/`llm`/`spi`) |
...
>
> **런타임 셀렉터**: 기본적으로 에이전트는 `claude-code` 백엔드(Claude Agent SDK + `claude` CLI)로 실행된다. 비-Claude·오픈 모델(OpenAI 호환 게이트웨이)을 쓰려면 `HuginnAgent.spec.agent.runtime: huginn-self` 로 선택한다. 설계·SPI 계약은 [`docs/design/muninn-pluggable-agent-runtime.md`](./docs/design/muninn-pluggable-agent-runtime.md).

## Quick start

### 로컬 풀스택 — `make run-kind` (루트)
...
```bash
make run-kind                       # kind 생성 → 이미지 3종 빌드/적재 → metaDB → helm install (Podman 기본)
# (선택) 자격이 있으면 코파일럿/agent 까지 활성화:
CLAUDE_CODE_OAUTH_TOKEN=... make run-kind
make status                         # 배포 상태
make down                           # kind 클러스터 삭제(정리)
```

> 이름은 같지만 `huginnOperator` 의 `make run-kind` 와 다르다: operator 의 `run-kind` 는 operator 를
> 클러스터 **밖**(host `go run`)으로 띄우는 빠른 컨트롤러 개발 루프이고, 루트의 `make run-kind` 는
> operator 까지 helm 으로 클러스터 **안** 에 배포한다.
...
### huginnSelfRuntime (옵션 런타임 — Go)

```bash
cd huginnSelfRuntime
go build ./... && go vet ./... && go test ./...   # runtimeapi·internal/strutil 테스트
# 이미지 (Makefile/CI 미배선 — 직접 빌드):
podman build -t ghcr.io/kimsoungryoul/muninn/huginn-self:dev .
```

`huginn-self` 바이너리는 `run`(라이브) / `selftest`(오프라인 배선 점검, claude-code selftest 와 출력 동형) 모드를 가진다. 빌드 시스템 배선(Makefile/루트 위임/CI publish)은 후속 작업이다.
