...
| `A2A_ALLOWED_HOSTS` | — | 클라이언트(`send_task_to_a2a_agent`)가 호출할 수 있는 host(또는 host:port) allowlist. 설정 시 그 목록만 허용. 미설정이면 https 강제 + 메타데이터/링크로컬 IP 차단(SSRF 가드). 로컬 더미 데모는 `localhost:4010` 추가. |
