...
| `TaskState` | `RunStatus`(`task-mapper.ts:statusToA2AState`) | `input-required` ≡ `awaiting`(AwaitingApproval) |
...
| `task-mapper.ts` | `statusToA2AState` / `runVmToTask` / `runVmToStatusUpdate` / `latestRun` / `isStreamFinal` (순수) |
...
| `MUNINN_API_TOKEN` / `MUNINN_OIDC_*` | — | 서버 라우트 인증은 muninn API 와 **단일 체계**(`lib/auth.ts` `requireAuth`): 정적 토큰 비교 또는 OIDC JWT 검증. 콘솔 우회 없음(머신 경로 — 토큰 필수). 둘 다 미설정이면 dev 모드(허용+경고). `tasks/cancel` 은 `requireOperator`(OIDC operator group 강제 환경에서 운영자 JWT 만 통과). |
...
| `A2A_BEARER` | — | 클라이언트가 대상에 보낼 bearer 토큰(가드 통과 URL 에만 첨부). **유일한 토큰 소스** — 모델 파라미터로는 받지 않는다(LLM 컨텍스트에 토큰 노출 방지). |
| `A2A_PUBLIC_BASE_URL` | — | **운영 권장.** Agent Card 의 `url` 에 쓸 신뢰 가능한 고정 외부 base URL. 미설정 시 프록시 헤더/host 를 형식검증 후 사용(host 헤더 스푸핑에 취약할 수 있어 운영에선 명시 권장). |

## V2 — muninn 을 A2A 서버로 (kind/클러스터 연결 시)

```bash
export MUNINN_A2A_ENABLED=1            # 서버 라우트 활성(필수)
# 인증: 로컬 dev 는 MUNINN_API_TOKEN/MUNINN_OIDC_* 미설정 시 dev 모드로 허용(경고 1회).
# 운영은 muninn API 와 동일 토큰으로 -H 'authorization: Bearer <MUNINN_API_TOKEN 또는 OIDC JWT>' 를 붙인다.
# 1) 에이전트 레지스트리 / 카드
...
> `message/send`·`message/stream` 은 `k8sEnabled()` 가 true 여야 실제 위임된다(아니면 `INTERNAL_ERROR(-32603)`, message `위임 실패: k8s-disabled`).
> 인증: muninn API 와 단일 체계 — `MUNINN_API_TOKEN` 또는 OIDC 가 설정된 환경에서는 `Authorization: Bearer <token>` 필수(콘솔 우회 없음), 미설정이면 dev 모드 허용. `tasks/cancel` 은 운영자 수준(`requireOperator`).
