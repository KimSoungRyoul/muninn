...
// 링크 href 화이트리스트 — http(s)/mailto 만 허용(javascript:, data:, vbscript: 등 차단).
// escapeHtml 이 이미 한 번 돈 뒤의 문자열을 받으므로 &amp; 를 복원해 스킴을 판정한다.
function safeHref(raw) {
  const url = raw.replace(/&amp;/g, "&").trim();
  if (/^(https?:\/\/|mailto:)/i.test(url)) return url;
  // 스킴 없는 상대/앵커 경로(/, #, ./)는 허용. 그 외(스킴 포함)는 차단.
  if (/^[/#]/.test(url) || /^\.\.?\//.test(url)) return url;
  if (!/^[a-z][a-z0-9+.-]*:/i.test(url)) return url; // 스킴이 전혀 없으면 상대경로로 간주
  return null; // 위험 스킴 → 링크 무력화
}
...
function mdToHtml(src) {
  ...
  // href 는 화이트리스트(safeHref)+속성 escape(escapeAttr). 위험 스킴은 링크 없이 텍스트만 남긴다.
  s = s.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, text, url) => {
    const href = safeHref(url);
    if (!href) return text; // javascript:/data: 등 → 평문 텍스트로 강등
    return `<a class="md-a" href="${escapeAttr(href)}" target="_blank" rel="noopener noreferrer">${text}</a>`;
  });
  s = s.replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>');
  s = s.replace(/(^|[^*\w])\*([^*\n]+)\*(?!\w)/g, '$1<em>$2</em>');
...
function MarkdownView({ src, className }: any) {
  return (
    <div
...
// ============== Editor modal ==============
function MarkdownEditor({ open, title = "문서 편집", filename, value = "", onSave, onClose, hint }: any) {
...
export { mdToHtml, MarkdownView, MarkdownEditor };
