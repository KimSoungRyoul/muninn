...
sequenceDiagram
    ...
    G->>API: POST /api/hooks/{app} (alert payload)
...
- **Webhook**: Grafana/Airflow/ArgoCD 가 `POST /api/hooks/{app}` 으로 alert 를 보낸다.
...
1. 에이전트(runner.py)가 위험 작업 직전 `POST /api/runs/{id}/report` 에 승인 요청을 보낸다.
...
- `POST /api/runs/{id}/report` — `step`/`cost`/`tokens`/`output`(Agent→API 소유 필드만 PATCH).
- `POST /api/runs/{id}/recall-report` — `recalledMemoryIds[]`(id, score, reason).
