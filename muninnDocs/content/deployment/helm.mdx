...
Muninn DevOps Agent Platform 을 Kubernetes 에 배포하는 Helm chart 다. 소스는 [deploy/helm/muninn](https://github.com/KimSoungRyoul/muninn/blob/main/deploy/helm/muninn/README.md).
...
<Callout type="info">
**PostgreSQL 은 이 chart 가 설치하지 않는다.** metaDB 는 외부 인스턴스를 쓴다 — 권장 경로는 CloudNativePG(CNPG)로 provision 후 그 연결 Secret 을 `metaDb.existingSecret` 으로 가리키는 것이다([Quickstart](/deployment/quickstart) 참고). `metaDb.enabled=true` 면 **muninn-web(= Muninn API)** 에 `DATABASE_URL`(Secret 의 `uriKey`)이 주입된다. 검색은 텍스트(to_tsvector/ts_rank_cd)만 쓰므로 **pgvector·확장은 불필요**하다. (`externalPostgresql.*` 는 미구현 소비자용 legacy 메타 Secret.)
</Callout>

## 사전 요구

- Kubernetes 1.27 이상, Helm 3.x 이상
...
operator/web 이미지는 **CI 미발행**이다 — 기본 이미지 값 그대로면 `ImagePullBackOff` 가 난다. 로컬 빌드 후 `kind load` + `--set *.image.*` override 가 필요하다(아래 "로컬 이미지로 kind 설치" 또는 루트 `make run-local`). agent Job 의 보고/메모리 endpoint(`MUNINN_API_ENDPOINT`/`MUNINN_MEMORY_ENDPOINT`)는 `web.enabled=true` 일 때 chart 가 이 릴리스의 muninn-web Service 로 배선한다(namespace 무관). `web.enabled=false` 면 operator 가 코드 fallback 을 쓰므로 보고가 동작하지 않는다.
</Callout>

### 로컬 이미지로 kind 설치 (Podman)

**권장: 루트 `make run-local`** — kind 생성 + 이미지 3종 빌드/적재 + metaDB + 이 chart 설치를 한 번에 한다. 완전수식 이미지 이름(`ghcr.io/kimsoungryoul/muninn/*`)으로 빌드해 podman 의 `localhost/` 접두 문제를 피하고, helm 값을 자동 배선한다(아래 수동 절차는 내부에서 일어나는 일을 풀어쓴 것).
...
for img in huginn-operator muninn-web; do
  ...
  KIND_EXPERIMENTAL_PROVIDER=podman kind load image-archive /tmp/$img.tar --name <cluster>   # <cluster> = kind 클러스터 이름 (예: muninn)
done
...
kind 에 load 한 로컬 이미지는 `pullPolicy: IfNotPresent`(기본)면 노드에 있을 때 재pull 하지 않는다. (과거 `huginnOperator/.dockerignore` 가 buildah 에서 `cmd/main.go` 를 누락시키던 문제는 명시적 제외 패턴으로 수정됨 — 이제 `podman build huginnOperator` 가 우회 없이 동작한다.)

## 자격(Secret) — 절대 커밋 금지

<Callout type="error">
"Auth is env(Secret)-only" 원칙에 따라 토큰/키는 chart/values·매니페스트에 절대 넣지 않는다. 아래처럼 환경변수를 참조해 Secret 으로만 주입하라.
</Callout>
...
CRD 는 Helm 의 `crds/` 디렉토리로 관리된다(원본: [huginnOperator/config/crd/bases/](https://github.com/KimSoungRyoul/muninn/tree/main/huginnOperator/config/crd/bases)).
...
CRD 자체(스키마·필드)는 [CRD 개념](/concepts/crds)과 [CRD 예제](/reference/crd-examples)를 참고하라.
...
전체 목록은 [values.yaml](https://github.com/KimSoungRyoul/muninn/blob/main/deploy/helm/muninn/values.yaml) 참고.
