...
> **상태**: Draft v0.4 · 2026-06-17 (Runtime selector 백엔드 plug 경계 §2.7, 도구 자격 백엔드 무관 주입 §2.5, 홈 분리·네이밍/PVC 리네임 개편 §2.6, §2.6 기각 테이블에 selector 조건부 채택 반영 — `muninn-pluggable-agent-runtime.md` 연동) · v0.3 · 2026-06-12 (멀티테넌시 admission `workspaceId==namespace` 강제, AwaitingApproval→Running 복귀, `MUNINN_WORKSPACE` 주입, CRD v1 보류 정책 반영)
...
> **`effectiveRuntime`(selector §2.7, 신설 — `HuginnIssueStatus`)**: 위 표는 `HuginnRunStatus` 소유권이고, runtime 동결 필드는 **`HuginnIssueStatus.effectiveRuntime`** 에 둔다. 소유자 = **Operator(첫 `createRun` 시 1회 기록, 이후 read-only)** — `HuginnRunStatus` 의 `maxStep`/`maxCostUsd`(Operator 생성 시 1회) 와 동일 패턴. attempt 간 백엔드 일관성을 보장해 cross-backend resume 손상을 막는다(§2.7).
...
**도구 자격(tool credential) 주입 — 백엔드 무관 (개편 대상).** 자격 env 는 두 범주다: (a) **LLM 인증**
(`ANTHROPIC_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN`/`MUNINN_API_TOKEN`), (b) **도구 자격**(`GITHUB_PAT`,
`ARGOCD_SERVER`/`ARGOCD_AUTH_TOKEN`, `GRAFANA_TOKEN`, `KUBECONFIG`/in-cluster SA 등 — muninnWeb 앱 설정 탭 입력).
**현재 `buildJobTemplate` 은 `GITHUB_PAT` 만 실제 주입**하고, ArgoCD/Grafana 자격과 **`spec.bindings`
(→`inheritedBindings`)는 operator 가 읽지 않는 데드필드**다(Job env 로 안 감; muninnWeb 자격 폼도 mock).
개편(하위호환 비필수): (1) operator 가 `agent-secrets` 의 도구 자격 키를 env 로 주입, (2) `ToolBinding` 이
`instance`+`config` 만 갖고 **자격 필드가 없는 모순**을 **`ToolBinding.secretRef` 추가**(자격을 CRD 에 둬 webhook 순수
검증 가능; `platform_tool` 레지스트리 안은 `instance` 와 책임 중복으로 폐기)로 해소해 operator 가
`inheritedBindings` 순회 → `secretRef` → 도구별 표준 env(`ARGOCD_AUTH_TOKEN`/`GRAFANA_TOKEN` 등) 주입. 이 주입은 **백엔드 무관**(claude-code·huginn-self 공통 SPI)이며 MCP
가용 여부와 독립 — 자격 *사용* 방식만 백엔드별로 다르다(MCP vs CLI/HTTP). 상세는
[`muninn-pluggable-agent-runtime.md`](./muninn-pluggable-agent-runtime.md) §2.1(b)/§2.7. claude-code·huginn-self
모두 부팅 시 `configure_auth` 동등 단계로 env→CLI 자격(git/gh/argocd/kubectl)을 구성한다.
...
- **비면(레거시 JobTemplate)** PVC 루트를 마운트 — 기존 동작 보존. (단, PVC 리네임 개편(아래) 채택 시 `pvc-claude-*` 가 사라지므로 이 레거시 폴백은 신규 배포에서 무의미 — 리네임과 함께 정리한다.)
...
**개편(selector 도입 시, 하위호환 비필수 — `CLAUDE.md` 규약).** subPath 메커니즘 자체는 백엔드 무관하게 유지하되,
claude-편향 네이밍·경로를 정리한다(프로토타입이라 운영 데이터 유실 위험 없음 → 점진 호환 불요):
- **HOME 분리**: claude-code = `~/.claude`(`claudeMountPath`), huginn-self = `~/.huginn`(`huginnMountPath` 신규).
  `expandPodSpec` 이 `VolumeMount.MountPath` 만 runtime 별 분기. **initContainer 는 무변경** — PVC 루트의
  `<issue>/` subPath 를 만드는 것이라 mountPath 와 독립이다(main 이 `.claude`/`.huginn` 어디에 붙여도 같은 디렉토리).
- **네이밍 리네임**: `claudeStoreInitPath`/`CLAUDE_HOME_DIR`/`claude-home-init` 컨테이너 이름은 백엔드 무관인데
  claude-편향 → `agentStoreInitPath`/`AGENT_HOME_DIR`/`agent-home-init`. PVC 명 `pvc-claude-<app>` →
  `pvc-agent-<app>`. 기능 무변경, 호환 레이어 불요.
- 상세·SPI 는 [`muninn-pluggable-agent-runtime.md`](./muninn-pluggable-agent-runtime.md) §2.4/§2.7.
...
| raw Messages API + 자체 루프 (**claude-code 단독 교체**) | 기각 | 에이전트 루프·도구 실행기·resume 재발명 = 더 큰 복잡도 |
| raw 루프 + 자체 모델 (**claude-code 와 selector 공존**) | **조건부 채택** | 위 기각은 "단일 모델 교체" 전제에서만 유효. **claude-code 를 기본 유지**한 채 *비-Claude 자체 모델 실행*용 백엔드(`huginn-self`)를 opt-in selector 로 추가하면 정당 — §2.7 + [`muninn-pluggable-agent-runtime.md`](./muninn-pluggable-agent-runtime.md) |
...
→ **claude-code 실행 모델(self-host + Agent SDK in K8s Job)은 기본값으로 유지가 정답.** 위 "raw 루프 단독 교체"
기각은 그대로 유효하다. 단, **게이트웨이/모델 우려는 무효**다: claude CLI 가 `ANTHROPIC_BASE_URL`/
`ANTHROPIC_AUTH_TOKEN`/Bedrock·Vertex env 를 honor 하므로 *Claude 모델을* 사내 게이트웨이로 돌리는 데는 자체
백엔드가 필요 없다(operator 가 env 주입). 자체 백엔드(`huginn-self`)는 **비-Claude 자체 모델 실행**이 목표일
때만 selector 로 추가하며(§2.7), claude-code 와 동일 SPI 를 만족한다. runner.py 의 응집도 문제(루프+보고+
HITL+시그널 혼재)는 *구조 변경이 아니라 모듈 분리*로 푸는 **후속 리팩터링**으로 남긴다(동작 불변).
...
> huginn-self 백엔드(§2.7)는 **OAuth 로 Messages API 직접 호출 불가** → API 키/게이트웨이 Bearer 필요.

### 2.7 Runtime selector — 실행 백엔드 plug 경계 (operator 측 계약)

`AgentSpec.Runtime`(현재 기본 `claude-code`, enum 제약 없는 **죽은 필드**)을 enum 으로 승격해 **HuginnRun 실행
백엔드를 선택**하게 한다. 선택축은 **모델**이다 — Claude 모델 → `claude-code`(현행, 고성능), 비-Claude 자체/오픈
모델(사내 vLLM·NAMC 등) → `huginn-self`(자체 Go 루프, Messages/OpenAI-호환 API 직접). 성능은 claude-code 우위가
자명하므로 그것을 기본 유지한 채 selector 로 공존시킨다(데드필드를 "살리기"가 아니라 selector 1급 축으로 **재설계**).

```go
// +kubebuilder:validation:Enum=claude-code;huginn-self
// +kubebuilder:default=claude-code
Runtime string `json:"runtime,omitempty"`   // v1beta1·v1 양쪽
```

**operator 측 계약(이 문서 소관):**
- **분기점 = `buildJobTemplate`**: runtime 보고 image/command/HOME 마운트경로/신규 env 를 가른다. `command` 상수
  `agentSkillCmd` 가 2곳(`helpers.go`/`huginnrun_controller.go`)에 하드코딩 → 둘 다 분기로 교체.
- **`expandPodSpec` 고정 필드 공유**: non-root uid 1000 · restartPolicy=Never · 컨테이너 이름 · serviceAccountName
  (`huginn-agent` SA, in-cluster RBAC)은 백엔드 무관 공유. **HOME mountPath 만** runtime 별 분기(§2.6 개편).
- **이미지 결정 개편(하위호환 비필수)**: required `agent.image` 를 **제거**하고 operator config flag
  (`--claude-code-image`/`--huginn-self-image`)로 runtime 별 기본 이미지 결정, `agent.image` 는 선택 오버라이드만 →
  runtime↔image 모순을 구조적 제거. webhook 으로 오버라이드 정합 검증(순수 함수 유지, §4).
- **JobTemplate immutability 와 정합(중요)**: `buildJobTemplate` 은 Issue 가 아니라 **attempt(Run)마다** 호출되므로,
  진행 중 Issue 의 runtime 을 바꾸면 attempt N+1 이 새 백엔드로 뜨면서 직전 attempt 의 `sessionId` 를 상속 →
  **cross-backend resume 손상**(transcript 포맷 비호환). 해소: 첫 attempt 시점 runtime 을 `HuginnIssue.status.effectiveRuntime`
  으로 **동결**하고 이후 attempt 는 그 값 사용(Agent.runtime 변경은 *다음 Issue* 부터) + `withResumeSession` 이 백엔드
  일치 시에만 sessionId 주입.
- **상태/보고 계약 불변**: 두 백엔드 모두 §2.2 status 소유권(Agent→API 필드만 patch)·report/recall·HITL·terminal
  보고 1회·dry-run 을 동일하게 만족(공통 SPI). operator·API·UI 는 백엔드를 구분하지 않는다.

전체 SPI·huginn-self 내부 설계(agent loop/tool/session/cost)·conformance·TCO 는
[`muninn-pluggable-agent-runtime.md`](./muninn-pluggable-agent-runtime.md). **huginn-self 는 opt-in 실험**으로 출시하고
claude-code 를 기본 유지한다.
...
5. §5 "Claude Agent SDK 로 루프 시작" → **"runtime 백엔드(claude-code|huginn-self)가 루프 시작"** 일반화, §5.3/§5.4 의 "SDK 계약" 을 백엔드 SPI 로 추상화(본 문서 §2.7, `muninn-pluggable-agent-runtime.md`).
6. §5.1 "도구 자격(secretKeyRef)" 을 **백엔드 무관 주입 + `configure_auth` 구성** 계약으로 구체화, `spec.bindings` 자격 필드 부재 모순을 스키마 재설계로(본 문서 §2.5).

> **개편 방침(`CLAUDE.md` 규약).** 위 §2.5~§2.7 의 변경(Runtime enum·`agent.image` 제거·홈 분리·claude-편향 네이밍/PVC 리네임·bindings 스키마 재설계)은 **하위호환을 목표로 하지 않는다** — 프로토타입이라 운영 데이터 유실 위험이 없으므로 점진 호환 레이어보다 깔끔한 재설계/제거를 택한다.
