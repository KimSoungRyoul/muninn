...
- **HITL 승인 게이트**: `requireApproval` 시 위험 작업 전 승인 요청(§2.2 이중 적재) → `AwaitingApproval` → 폴링으로 **`Approved`/`Rejected` 종결을 처리**. 매핑: rejected→`rejected`, expired→`expired`, timeout→`aborted`.
  - **`Expired` 도달성(정정).** web `lib/incidents.ts:approvalState()` 가 `expiresAt<=now` 시 표시값을 "Expired" 로 **lazy 계산**해 runner 의 폴링 GET 경로(`runView`)에 노출하도록 **이미 배선돼 있다** — 즉 `runner.py` 의 "Expired 도달 불가" 주석은 **stale 이 맞다**(코드 확인). 다만 v0.3 의 "실제 도달 가능" 단정은 **두 가지를 빠뜨렸다**: (1) **메인스펙 §6.4 도 동반 stale**("만료 데몬 미구현, 후속") — 같은 PR 에서 함께 정정해야 함(§10-2). (2) **5400s race(미해결)**: runner wall-clock 타임아웃(`MUNINN_APPROVAL_TIMEOUT` 기본 5400s)과 web 승인 TTL(`approvalTtlMinutes` 기본 90m=5400s)이 **같은 값**(operator `helpers.go` 가 "단일 소스"로 정합)이라, 폴링이 web 의 lazy "Expired"(→`expired`)를 먼저 보든 자신의 wall-clock 만료(→`aborted`)를 먼저 보든 **비결정적**이다. 따라서 `terminalKind=expired` 는 *현재 타이밍 의존*이며, §8 conformance 의 expiry 라운드트립을 결정적 assertion 으로 두려면 **두 타임아웃의 권위 순서를 먼저 결정**해야 한다(권장: runner 폴링 타임아웃을 web TTL 보다 길게 두어 web 의 "expired" 가 권위를 갖게 — §11 Open Question). 두 백엔드 공통 계약이므로 huginn-self 도 동일.
...
**수요 검증 게이트(신설 — §9-4 PoC 선결)**: 동기 재정의(§0.2)는 정직하나 "비-Claude 모델 실행 **수요가 실재**" 는 아직 미입증이라, 그대로 두면 selector 는 *죽은 필드(`AgentSpec.Runtime`) 살리기의 사후 정당화* 비판을 못 벗는다. 따라서 PoC 착수 전 **구체적 수요 1개 이상**을 명시한다 — *어느 팀/워크로드가 어떤 사내 모델(예: 특정 vLLM·NAMC 모델)을, 그 모델이 통과해야 할 최소 에이전트 태스크와 함께* 요구하는지. 이게 비면 본 설계는 "**지금 구현하지 말 것을 권고하는 ADR**" 로 포지셔닝한다(아래 수명 결정과 일관).
...
- **HITL expiry(조건부)**: `expiresAt 경과→expired` 라운드트립은 **5400s race 해소(§2.6 타임아웃 권위 순서 결정) 후에만** 결정적 assertion 으로 추가. 그 전 단계에서 결정적으로 검증 가능한 것은 거절 선점(→`rejected`)·wall-clock 만료(→`aborted`)뿐.
...
2. **`runner.py` / 메인스펙 §6.4 (동반 정정)**: `gate_approval` "Expired 도달 불가" 주석은 **stale 이 맞다**(web `incidents.ts:approvalState()` 가 lazy "Expired" 를 이미 표면화 — 코드 확인). 단 **메인스펙 §6.4 도 같은 stale 서술**("만료 데몬 미구현, 후속")을 갖고 있어 **함께 정정**해야 하며, runner wall-clock 타임아웃 == web TTL(둘 다 5400s) **race** 를 해소(타임아웃 권위 순서 결정, §2.6·§11)하기 전엔 `terminalKind=expired` 가 비결정적임을 주석/스펙에 명기. (단순 "주석만 정정" 이 아니라 race 결정 + 메인스펙 동반.)
...
| # | 질문 | 영향 | 참조 |
|---|------|------|------|
| Q1 | **compaction 방식** (A)요약 LLM 호출 vs (B)sliding window+pin | huginn-self GA 가능 여부(최우선) | §6.1 |
| Q2 | **수요 검증** — 어느 팀/워크로드가 어떤 사내 모델을 요구하는가(구체 1개) | selector 정당성(없으면 ADR=보류 권고) | §7 |
| Q3 | **cost 추정 불가 정책** fail-closed vs fail-open (`MUNINN_COST_POLICY`) | guardrail 집행 | §2.3a·§4.4·§6.3 |
| Q4 | **status.tokens 의미** + 게이트웨이 usage 미제공 시 추정 계약 | cross-backend 비교 | §2.3a |
| Q5 | **모듈 토폴로지** (a) 독립 모듈 확정 + 순수 DTO 미러 범위 | 슬림 바이너리·drift | §4.5 |
| Q6 | **selector 수명** 영구 옵션 vs claude-code sunset 브리지 | TCO 비가역성 | §7 |
| Q7 | **HITL expiry 타임아웃 권위 순서** (runner 폴링 vs web TTL, 현재 5400s 동률 race) | `terminalKind=expired` 결정성·conformance | §2.6·§8 |
| Q8 | **codegen 채택 여부**(openapi/proto→Go+TS) vs conformance 영구 감시 | drift 차단 실효·TCO | §4.5·§8 |
| Q9 | **`ToolBinding.secretRef` 키맵 스키마** + webhook 검증 규칙 | 도구 자격 흐름 | §2.7·§10-8 |
| Q10 | **`MUNINN_TOOL_ALLOWLIST` 기본값·형식** + 서브커맨드/플래그 단위 차단 규칙 | 보안(인젝션) | §6.2-1 |
| Q11 | **HITL 게이트 위치** — 루프 진입 전(현 runner.py 한계) vs plan 산출 후 | 승인 품질 | §2.6 |

> **종합**: 설계 *방향*(claude-code 기본 + huginn-self opt-in, 공통 SPI, effectiveRuntime 동결, compaction·보안 GA gate)은 견고하다. 그러나 **Q1·Q2·Q3·Q7** 은 구현 착수 전 필수 결정이며, 특히 **Q2(수요)** 가 비면 본 문서는 "지금 구현하지 말 것을 권고하는 ADR" 로 읽혀야 한다(§7 과 일관). 즉 *의사결정을 명문화할 가치*는 충분하나 *지금 구현할 가치*는 수요 게이트에 달렸다.
