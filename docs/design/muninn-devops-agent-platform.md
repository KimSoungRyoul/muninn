...
```
Grafana alert ─▶ Muninn API(Gateway) ─▶ (정규화·dedup) ─▶ K8s API ─▶ HuginnSession CR
...
- **Application**: 운영 대상. `kind: triton|fastapi|airflow|other`, `output: pull_request|issue`, `repo`, 집계지표 `runs24h/failed24h/cost7d`(§8.1).
...
3. 신규/재발이면 API 가 **K8s API Server 에 `HuginnSession` CR 을 생성**(client-go/Python client)한다. 원본 alert 는 Secret `{session}-event`, 정규화 구조는 `spec.event` 에 둔다.
...
- `status.conditions[]`(Ready 등 K8s 표준), `status.activeSessions`(Operator 가 reconcile 주기마다 `phase∈{Pending,Running,AwaitingApproval}` 세션 수로 계산; §8.4).
...
세션 내부 실제 실행 1회. **MVP 는 K8s Job 으로 실행**(native `backoffLimit`·`activeDeadlineSeconds`·`ttlSecondsAfterFinished`·`podFailurePolicy` 활용).
...
- **삭제/정리**: Run/Session 은 `ownerReferences` 로 cascade GC. 진행 중 Pod 의 graceful shutdown 이 필요하면 finalizer 추가(§11). conditions 로 전이 사유(거절/타임아웃/비용초과)를 기록한다.
- **모든 CR(App/Session/Run)** 의 `status.conditions[]` 는 `{type,status,reason,message,lastTransitionTime}` 표준 구조.
...
sequenceDiagram
    ...
    alt 신규 fingerprint 또는 재발
        API->>K: create HuginnSession (spec.event + goal), Secret {sess}-event
        K-->>OP: watch 이벤트
        ...
        alt output=pull_request & approvalTriggers 충족
            A->>API: request-approval(reasons) → Session.phase=AwaitingApproval
        ...
        API->>R: dedupCount++ (세션 미생성)
    end
```

### 4.1 핵심 규칙
- **1 Event = 1 Session, 1 Session = N Run**(retry/replay).
...
```jsonc
// HuginnSession.spec.event (정규화형) — 원본은 Secret {session}-event 에 보존
{
  "id": "e_3f8a91",
...
- **재발 판정**: 동일 fingerprint 의 **가장 최근 Session 이 `Running`/`AwaitingApproval`** 이면 새 세션 생성 안 함(카운트만). **`Succeeded`/`Failed`/`Cancelled`** 이면 재발로 보고 새 세션 생성.
- **영속화**: webhook 수신 시 `event` 테이블에 dedupCount 기록(감사). Session 은 최종 dedupCount 를 읽는다.
...
### 5.1 컨테이너 부팅 시퀀스
```
1. PVC(~/.claude) 마운트 → Claude Code "프로젝트 설정/스킬/세션"만 공유.
...
2. 컨텍스트 주입(env/ConfigMap/Secret/파일):
   ...
   - MUNINN_GUARDRAILS                 : maxIterations/maxCostUsd(상속)
   - ANTHROPIC_API_KEY                 : secretKeyRef(agent-secrets)  ← env 로만
...
- **C. `ReadWriteOncePod` + 장기 세션 Pod 1개**(컨텍스트 재사용 시)

**MVP = A(앱별 PVC) 또는 B**. 동일 앱의 동시 Run 은 세션 디렉토리를 `~/.claude/run/{runId}` 로 분리해 충돌을 줄인다.
...
- **`output=issue` 는 자동 발행(승인 불필요)**.
- **집행**: 에이전트가 조건 감지 → `POST /sessions/{id}/request-approval {reasons:[{type,detail}]}` → Operator 가 Session/Run `AwaitingApproval`. MVP 는 에이전트 코드 내 하드코딩 체크, 이후 정책 중앙화.
...
### 8.4 status 파생값
`activeSessions` 는 Operator 가 reconcile(기본 30s)마다 계산하는 transient 값(SQL 영속 불요, 필요 시 Redis 30s TTL 캐시).
...
### 8.6 goal 영속화
`HuginnSession.spec.goal` 은 event 단위 불변 컨텍스트. 운영 감사 필요 시 `event.goal VARCHAR` 추가, 아니면 CRD-only transient 로 둠(설계 의도 명시).
...
- [ ] Operator: 단일 namespace, Session→Job 생성, GC
...
3. **에이전트 격리 단위**: Run 당 Job(짧고 깨끗) vs Session 당 장기 Pod(컨텍스트·`~/.claude` 캐시 재사용). 그림의 PVC 공유는 후자 시사 → §5.5 와 연계.
4. **세션 동시성 상한 / 우선순위 큐**(critical 우선).
