...
> **구현 현황(MVP)**: 위 표·다이어그램은 *목표 설계*다. 현재 구현은 단순화돼 있다 — **Muninn API·Muninn Memory Service 는 별도 서비스가 아니라 muninnWeb(Next.js) 한 앱이 겸한다**(§0 콜아웃; `app/api/**` 게이트웨이 + recall/store). **Redis 와 FastAPI·별도 Memory Service 는 구현되지 않았다**: dedup 자체가 MVP 미구현(§4.4), 메모리는 muninnWeb + 외부 **postgres**(Drizzle)에 저장하고 **텍스트 검색**(`to_tsvector`/`ts_rank_cd`)만 쓴다 — 임베딩·**pgvector·확장 불필요**(§7.4). 따라서 다이어그램의 `API`/`MEM` 노드는 muninnWeb 하나로, `REDIS` 노드와 `PG` 의 pgvector 는 현 구현에 없다고 읽어라.
...
> **구현 현황(MVP)**: **dedup·severityThreshold gate·원본 payload 보존(Secret `{issue}-event`)은 모두 미구현(후속)**이다. 현 `POST /hooks/{app}`(muninnWeb `app/api/hooks/[app]/route.ts`)는 알림마다 새 `HuginnIssue` 를 생성하며(재발 카운트·`dedupCount` 갱신 없음), severity gate 는 하드코딩값이고 원본 alert payload 는 보존하지 않는다. Redis 도 없다. 알림 폭주 보호는 Issue 당 `retryPolicy.maxRuns` 상한에만 의존하므로, 운영 노출 전에 dedup 을 구현해야 한다.
...
> **구현 현황(MVP)**: 이 절은 *목표 멀티테넌시 모델*이다. **namespace=workspaceId 강제, workspace 멤버십 admission 검증, SQL RLS, NetworkPolicy egress 제한은 미구현(계획)**이다. 현 MVP 는 단일/지정 namespace(`MUNINN_NAMESPACE`)에서 동작하며, agent SA 의 자기 namespace 한정 Secret/ConfigMap read 만 실제로 강제된다(operator 의 namespace 한정 Role).
...
- **집행**: 에이전트가 조건 감지 → `POST /issues/{id}/request-approval {reasons:[{type,detail}]}` → **Muninn API 가** Run 을 `AwaitingApproval` 로 전이(`AwaitingApproval` 전이·`approval` 은 **API 소유 필드**다 — Operator 가 아니라 API 가 쓴다; operator-design §2.2). Operator 는 그 phase 를 보존만 한다. MVP 는 에이전트 코드 내 하드코딩 체크, 이후 정책 중앙화.
...
> **구현 현황(MVP)**: 승인 *기록*(Muninn API 의 approve/reject 라우트가 `status.approval` 을 merge-patch — `state`/`reasons`/`decidedAt`/`reason` 등)은 동작한다. 다만 **에이전트 측 승인 루프는 미구현(후속)**이다 — runner.py 에 `approvalTriggers` 감지·`request-approval` 전송·승인 폴링/일시정지가 없어 실 e2e 에서 `AwaitingApproval` 전이 자체가 발생하지 않으며, 승인 후 `phase` 를 `Running` 으로 되돌려 에이전트를 재개하는 경로도 아직 없다. 만료 집행(87분→Failed/Cancelled)도 계획이다.
...
> **구현 현황(MVP)**: 위는 *제안* 스택이다. 실제 구현은 **API/Gateway·UI·Memory 를 muninnWeb(Next.js, App Router) 하나로 통합**했다 — FastAPI·별도 Memory Service 없음(§2.1 콜아웃). **Storage 는 외부 postgres 단일**(metaDB=memory 겸용, **텍스트 검색만 → pgvector·확장 불필요**), **Redis 는 미사용**(dedup MVP 미구현). Operator 는 Go(controller-runtime) 그대로다.
